...
### TO-DO:
# 1. refactor one funct for the user inputs
# 2. Obtain the actual equations format separated
# 3. mk 3D graph
...
ulst = ['' for i in range(len(guv))]    # empty List of 2D-arrays
for uk in range(len(guv)):              # for every unknown [x1, ..., xN]
    UKM = CM.copy()                     # born base uknown matrix
    UKM[:, uk] = r         # replace values of uk col by r vector (real uk matrix)
    ulst[uk] = round(np.linalg.det(UKM) / np.linalg.det(CM), 4)     # Cramer's rule
